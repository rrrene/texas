module Texas
  module CLI
    # An instance of Texas::CLI::Runner is used to run a build process.
    #
    class Runner
      ...
      #
      # Example:
      #   Texas::CLI::Runner.new(%w(-w --no-color))
      #   Texas::CLI::Runner.new(:warnings => true, :colors => false)
      #
      ...
      def options_from_args(args)
        if args.is_a?(Hash)
          opts = Texas::CLI::OptionParser.new([]).parse
          args.each { |k, v| opts.send("#{k}=", v) }
          opts
        else
          Texas::CLI::OptionParser.new(args).parse
        end
      end
